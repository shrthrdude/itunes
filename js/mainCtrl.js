...
app.controller('mainCtrl', function($scope, itunesService){
  ...
  //Our controller is what's going to connect our 'heavy lifting' itunesService with our view (index.html) so our user can see the results 
  //they get back from itunes.
  ...
  //Now write a function that will call the method on the itunesService that is responsible for getting the data from iTunes, whenever the user clicks the 
  //submit button
  ...
  //Also note that that method should be retuning a promise, so you could use .then in this function.
    ...
    $scope.getSongData = function(){
      ...
      .then(function(songData){
        $scope.songData = songData.data.results;
        var myFinalArray = [];
        for (i =1; i < songData.data.results.length; i++){
          var myObj = {
            AlbumArt: $scope.songData[i].artworkUrl100,
            Artist: $scope.songData[i].artistName,
            Collection: $scope.songData[i].collectionName,
            CollectionPrice: $scope.songData[i].collectionPrice,
            Play: $scope.songData[i].previewUrl,
            Type: $scope.songData[i].kind
          }
          myFinalArray.push(myObj)
        }
  ...
  //If everything worked you should see a huge array of objects inside your console. That's great! But unfortunately that's not what ng-grid is expecting. 
  ...
  //is sort the data you got back to be an object in the following format.
    /*
      ...
      Type: "song"
  */
  //the iTunes API is going to give you a lot more details than ng-grid wants. Create a new array and then loop through the iTunes data pushing
  // into your new array objects that look like the above data.

    //Code here


  //Once you have that final data array, you simply need to put it on the scope (or more specifically on the scope as songData). 
  //Once you do this ($scope.songData = myFinalArray) then ng-grid will see that and populate the page.
